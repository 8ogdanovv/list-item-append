...
(function (factory) {
    ...
    rules = {
        ...
        "validate-lang": [
            function (v) {
                return $.mage.isEmptyNoTrim(v) || /^[a-z\u0590-\u05fe]+$/i.test(v) ;
            },
            $.mage.__('יש להזין ערך בעברית או אנגלית')
        ],
        "validate-mobile": [
            function (v) {
                return $.mage.isEmptyNoTrim(v) || (v.substring(0, 1) == '0' && v.substring(0, 3) * 1 > 19.9 &&  /^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4}$/im.test(v) || /^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{4}[-\s\.]?[0-9]{3}$/im.test(v));
            },
    ...
    $.extend(true, $.validator.prototype, {
        ...
        showLabel: function (element, message) {
            ...
            if ($('body').hasClass('_has-modal') || elem.data('prevent-scrollto')) window.validateScrollTo = false;
